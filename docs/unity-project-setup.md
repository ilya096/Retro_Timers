...
7. Проверка debug-режима смещения:
   - дефолтный режим MVP: `Preserve displacement`;
   - сдвинуть стоящего на плите клона и отпустить; он должен остаться на новом месте, если в записи в этот момент он стоял;
   - после вмешательства клон должен получить visual altered feedback;
   - в Unity Editor или development-сборке нажать `F3`, чтобы открыть debug-панель;
   - в debug-панели проверить `Debug clone mode` и `Clone altered`;
   - при необходимости нажать `Toggle clone mode`, чтобы сравнить с `Recover to recording`.
8. Проверка delayed spawn preview:
   - после первой отмотки новая итерация не должна появляться мгновенно;
   - до нажатия rewind ghost/preview на spawn-точке не должен быть виден;
   - после нажатия rewind текущая итерация должна скрыться;
   - в точке spawn должен быть виден полупрозрачный ghost/preview игрока;
   - над ghost/preview должен заполняться небольшой радиальный progress-индикатор без текста;
   - диаметр радиального индикатора не должен быть больше ширины персонажа;
   - основной HUD не должен показывать числовой countdown задержки;
   - при spawn должен быть короткий flash/pulse.
9. Проверка clone readability:
   - активный игрок должен быть самым заметным;
   - самый новый playback-клон должен быть заметнее старых;
   - старые playback-клоны должны оставаться видимыми, но более прозрачными;
   - altered-клон должен получить flash, затем слабый tint/outline без изменения age-alpha.
   - если клон отмотан после короткой ручной попытки, после конца записанного движения он должен стоять на последней позиции до конца таймера уровня, а не исчезать сразу.
10. Проверка clone-vs-clone playback drift:
   - первой итерацией поставить клона на платформу/плиту и отмотать;
   - второй итерацией перепрыгнуть первого клона без касания, пройти дальше и отмотать;
   - третьей итерацией наблюдать playback первых двух клонов;
   - второй playback-клон должен перепрыгнуть первого и продолжить записанный маршрут, не застревая на голове/теле первого playback-клона.
11. Проверка resolved path измененного клона:
   - первой итерацией поставить клона на нажимную плиту и отмотать;
   - второй итерацией перепрыгнуть первого клона, затем сдвинуть его с плиты и отмотать;
   - третьей итерацией наблюдать playback;
   - первый playback-клон должен уйти с плиты по измененному пути, записанному во второй итерации;
   - второй playback-клон не обязан физически толкать первого заново, потому что результат вмешательства уже сохранен в resolved path первого клона.
12. Проверка visual config:
   - на `Time Rewind Controller` в Inspector должен быть блок `Visual Config`;
   - текущие цвета игрока, клонов, delayed spawn preview и тестовых объектов должны быть доступны для экспериментов без правки кода.
